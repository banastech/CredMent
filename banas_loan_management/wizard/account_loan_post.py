...
class AccountLoanPost(models.TransientModel):
    ...
    def _default_journal_id(self):
        ...
        if loan_id:
            return self.env['banas.loan'].browse(loan_id).journal_id.id

    @api.model
    def _default_account_id(self):
        loan_id = self._context.get('default_loan_id')
        if loan_id:
            loan = self.env['banas.loan'].browse(loan_id)
            if loan.is_leasing:
    ...
    payment_journal_id = fields.Many2one('account.journal', string="Payment Method", required=True)
    ...
    def run(self):
        ...
        if self.loan_id.state != 'approved':
            raise UserError(_('Only loans in approved state can be posted'))
        self.loan_id.post()
        move = self.env['account.move'].create(self.move_vals())
        move.post()

        if self.loan_id.process_fee > 0 or self.loan_id.other_fee > 0:
            self.create_fees_invoice()
        if self.loan_id.sales_person_id and self.loan_id.commission_percentage > 0:
